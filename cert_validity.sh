...
extract_and_validate_certificate() {
  ...
  # Get the certificate content from the Kubernetes secret
  CERT_CONTENT=$(kubectl get secret "$SECRET_NAME" -n "$NAMESPACE" -o jsonpath="{.data.$CERT_KEY}")
...
for SECRET_NAME in "${SECRET_NAMES[@]}"; do
  extract_and_validate_certificate "$SECRET_NAME" "cert"
done
